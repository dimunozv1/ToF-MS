	...
	void addNumberstoVector(std::vector <double>& data) {
		std::random_device rd;

		// Initialize a random number generator
		std::mt19937 gen(rd());

		// Define a uniform real distribution between 0 and 1.5
		std::uniform_real_distribution<> dis(0.0, 1.5);
		bool add = int(dis(gen));
		if (add) {
			data.push_back(50);
		}
		std::cout << "Boolean value " << add << std::endl;
	}
		...
		if (runMode) {
			...
			try
			{
				...
				else
				{
					...
					{
						pulse_width = 12e-09;
					} // Assign default value for num1 //std::cout << "Using default value for pulse width: 12e-09" << std::endl;
			...
			vectorReads.open("..\\..\\data_testing\\vectorReads_freq_" + std::to_string(freq_Hz) +
				"_t_" + std::to_string(group_end) + "_Packs_" + std::to_string(max_packet) + ".txt");
			...
			HistogramGraphics histogram = initializeHistogramGraphics(window, data, 80, font);
			while (window.isOpen())
			{
				auto start = std::chrono::high_resolution_clock::now();
				...
				if (i == 100)
				{
					...
					time_vs_size << elapsed.count() << "\t" << histogram.HistoricalData.size() << "\n";
