	...
	int64_t processPacket(volatile crono_packet *p, bool print, timetagger4_static_info *si, timetagger4_param_info *pi, bool verbose, bool trial,
	...
	int main(int argc, char* argv[]) {
		 int freq_Hz, group_end,max_packet,pulse_width,verbose, trial;
		try {
		...
		std::vector<double> data; //max allocation 2e29 doubles
		...
		{    printf("Reading packets:\n");
			...
			else
			{
				...
				while (p <= read_data.last_packet)
				{
					...
					processPacket( p, print, &static_info, &parinfo, verbose,trial,data);
