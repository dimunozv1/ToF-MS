...
directory = 'C:/Users/Administrator/Documents/Diana/data_testing/Data/'
...
Loops = 2
startvalue = int(30000)
endvalue = int(1.06601e+07)
steps = int(0.1)
YAxislimit = 1.5
XAxislimit = 30100
 ...
 #(too long to fit into one line in pdf format)
    if open(directory + last_file(), "r").readable() and os.stat(directory+ last_file()).st_size != 0:
        ...
        #Just for seeing it while programming. Can be deleleted later on
            ...
            RefreshFileRead = list(map(float, lineread[:-1].split('\n')))
            ...
            ax.set_xlim([startvalue,XAxislimit])
