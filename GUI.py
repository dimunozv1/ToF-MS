...
def run_makefile():
    ...
    subprocess.run(makefile_command, shell=True,cwd=".\\")
...
def get_data_acq_window(Data_acquistion_entry):
    try:
        ...
        # Show a warning message box
        messagebox.showwarning("Invalid input", "Data acquisition window must be a number between 0 and 100. Value has been set to 40.")
    return value1
...
def get_runtime(run_time_entry):
    try:
        ...
        # Show a warning message box
        messagebox.showwarning("Invalid input", "Runtime must be a number greater than 0. Value has been set to 60.")
    return value1
def run_tofms():
...
Data_acquistion_label = tk.Label(window, text="Data acquisition window [0-100] (us):")
Data_acquistion_entry = tk.Entry(window)

run_time_label = tk.Label(window, text="Program run time [0-] (s):")
